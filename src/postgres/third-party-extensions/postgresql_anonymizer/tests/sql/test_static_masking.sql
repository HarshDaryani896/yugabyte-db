...
-- Remove all masking rules with a policy
ROLLBACK TO after_init;
SELECT anon.remove_masks_for_all_columns('anon');
SELECT COUNT(*)=0 FROM anon.pg_masking_rules;

-- Remove all masking rules with an empty policy
ROLLBACK TO after_init;
SELECT anon.remove_masks_for_all_columns('devtests');
SELECT COUNT(*)!=0 FROM anon.pg_masking_rules;
